...
def gaussian_2D(x, y, x0, y0, xA, yA, theta, A, B):
    """Elliptical(?) 2D Gaussian function"""
    # Convert angle from degrees to radians
    theta = np.deg2rad(theta)
    a = np.cos(theta)**2 / (2*xA**2) + np.sin(theta)**2 / (2*yA**2)
